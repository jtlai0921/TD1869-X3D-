...
def main(command_args):
    ...
    command_args.train_directory  訓練集所在的資料夾。這個資料夾下面，每個資料夾的名字代表label名稱，再下面就是圖片。
    command_args.validation_directory 驗證集所在的資料夾。這個資料夾下面，每個資料夾的名字代表label名稱，再下面就是圖片。
    command_args.labels_file 一個檔案。每一行代表一個label名稱。
    command_args.output_directory 一個資料夾，表示最後輸出的位置。

    command_args.train_shards 將訓練集分成多少份。
    command_args.validation_shards 將驗證集分成多少份。
    command_args.num_threads 執行緒數。必須是上面兩個參數的因數。

    command_args.class_label_base 很重要！真正的tfrecord中，每個class的label號從多少開始，預設為0（在models/slim中就是從0開始的）
    command_args.dataset_name 字串，輸出的時候的前綴。

    圖片不可以有損壞。否則會導致執行緒提前離開。
    """
